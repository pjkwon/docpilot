...
| 포맷 | 구조 정보 |
|------|-----------|
| HWPX · DOCX | 스타일 이름 및 폰트 크기 기반 헤딩 감지. HWPX는 다중 섹션 문서 전체 추출 지원 (글상자·각주·표 셀 포함) — 문서 생성 시에도 `section0.xml`뿐 아니라 모든 `section*.xml`의 플레이스홀더가 채워집니다. DOCX도 본문뿐 아니라 헤더·푸터(기본/첫 페이지/짝수 페이지 헤더 포함)의 플레이스홀더까지 채워집니다 |
...
정확한 검색 품질 벤치마크 수치는 모델마다·태스크마다 달라 여기서 단정하지 않습니다. 필요하면 [MTEB 리더보드](https://huggingface.co/spaces/mteb/leaderboard)에서 두 모델을 직접 비교하거나, 아래 "로컬 모델 검색 품질 실측" 절의 자체 벤치마크를 참고하세요.
...
### 로컬 모델 검색 품질 실측 (합성 코퍼스)

`tests/test_embed_quality_bench.py`로 측정. 15개 주제 클러스터(클러스터당 서로 다른 표현의 문장 6개, 총 90문장)로 구성한 합성 한국어 코퍼스에 클러스터당 자연어 질의 1개를 던져 Recall@6 / MRR / nDCG@6을 계산합니다. 클러스터 내 문장은 어휘 중복이 낮게 작성해 키워드 매칭이 아닌 의미 기반 검색 품질을 반영하도록 했습니다.

| 모델 | 차원 | Recall@6 | MRR | nDCG@6 |
|---|---|---|---|---|
| multilingual-MiniLM-L12-v2 | 384 | 0.544 | 0.856 | 0.601 |
| multilingual-e5-base (기본) | 768 | 0.600 | 1.000 | 0.692 |
| multilingual-e5-large | 1024 | 0.722 | 1.000 | 0.790 |
| BAAI/bge-m3 | 1024 | 0.656 | 1.000 | 0.734 |

- e5-base/e5-large/bge-m3는 MRR 1.0 — 15개 질의 모두 정답 클러스터 문장이 최상위 1위로 검색됨. 차이는 Recall@6(동일 클러스터 문장을 top-6에 얼마나 모으는지)에서 벌어지며 e5-large가 가장 안정적입니다.
- MiniLM은 크기가 작은 만큼 세 지표 모두 가장 낮습니다.
- `sentence_embed_fn`/`default_embed_fn`은 e5 계열이 권장하는 `"query: "`/`"passage: "` 접두사 없이 원문을 그대로 인코딩합니다(smart-docgen 실사용 방식과 동일). 접두사를 붙이면 e5 계열 점수가 더 오를 수 있습니다.
- 코퍼스가 90문장·15질의로 작아 절대 수치보다는 모델 간 상대 비교 참고용입니다. `uv run pytest tests/test_embed_quality_bench.py -s`로 재현 가능합니다.
